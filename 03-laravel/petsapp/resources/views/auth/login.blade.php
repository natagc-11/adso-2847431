...
@section('content')
    ...
    <form method="post" action="{{ route('login') }}">
        ...
        <fieldset class="fieldset w-sm bg-base-200 border border-base-300 p-4 rounded-box">
                ...
                <input type="email" name="email" class="input rounded-full w-full" placeholder="j@mail.com" />
                
                <label class="fieldset-label">Password:</label>
                <input type="password" name="password" class="input rounded-full w-full" placeholder="secret" />
                
                <button class="btn mt-4 p-6 rounded-full text-white bg-purple-800 w-full">Login</button>

                 <a class="underline mt-2 text-sm text--800 hover:text-purple-900 rounded-md focus:outline-none" href="{{ route('password.request') }}">
